    ...
    def test_0010_add_payment_profile(self):
        ...
        with Transaction().start(DB_NAME, USER, CONTEXT):
            ...
            with app.test_client() as c:
                ...
                # If website dosent allow to save payment profile
                websites = self.NereidWebsite.search([])
                self.NereidWebsite.write(websites, {
                    'save_payment_profile': False,
                })
                # request to add payment profile will give 404
                rv = c.post(
                    '/my-cards/add-card',
                    data={
                        'owner': 'Test User',
                ...
                # setting save_paymnet_profile to true
                self.NereidWebsite.write(websites, {
                    'save_payment_profile': True,
                })
    ...
    def test_0020_view_payment_profiles(self):
        ...
        with Transaction().start(DB_NAME, USER, CONTEXT):
            ...
            with app.test_client() as c:
                ...
                # Response of request will be 404
                rv = c.get('/payment-profiles')
                self.assertEqual(rv.status_code, 404)
